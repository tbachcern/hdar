...
public class HdaEntryParser {
  ...
  public static HdaEntry parseHdaWebsiteStringToHdaEntry(final String website) {
    ...
    hdaEntry.setId(StringUtil.getIdFromWebsiteStringString(website));
  ...
  static void setLinks(final HdaEntry hdaEntry, final Element divIdContent) {
    final Element divClassDownload = divIdContent.getElementsByClass("download").first();
    final Elements span = divClassDownload.getElementsByTag("span");
    for (final Element divClassSpan : span) {
      final HdaLink hdaLink = new HdaLink();
      final Elements spanElements = divClassSpan.getElementsByTag("a");
      if (spanElements.size() <= 0) {
        continue;
      }
      final String href = spanElements.attr("href");
